...
function create_archive() { 
	...
	if($_POST['insert']) {
		...
		if($_POST['archive_id']) {
			$temp_res = $wpdb->update('wp_journal_archives',
				array (
					...
					array( 'id' => $_POST['archive_id'])  
				);

			if($temp_res) {
				$message.="Journal Archive updated successfully";                
			} else {
				$message.="Journal Archive unable to update";
			}	
		} else {			
					...
					<form class="form-horizontal" id="archive_form" enctype="multipart/form-data" method="post" novalidate="novalidate">
						<div class="form-group">
							...
							<div class="col-sm-9">
								...
								<input type="hidden" name="archive_id" id="archive_id" value="<?php echo isset($edit_vals['0']['id'])?$edit_vals['0']['id']:''; ?>">
							</div>
						</div>
						<div class="form-group">
							<label for="journal_name" class="col-sm-3 control-label">Select Journal</label>
							<div class="col-sm-9">
								<select class="form-control" name="journal_name" id="journal_name" required onblur="convertToJournalSlug(jQuery('#journal_name option:selected').text())">								
									<?php 
									echo '<option value="">Select Journal</option>';
									echo '<optgroup label="Medical"></optgroup>';
									foreach ($temp as $key => $value) {										
										if($value['category_name'] == 'Medical') {
											if($edit_vals['0']['journal_id'] == $value['id']) {
...
<div class="form-group">
	<label for="journal_name" class="col-sm-3 control-label">Journal Slug</label>
	<div class="col-sm-9">							
		<input name="journal_slug" id="journal_slug" class="form-control" value="<?php echo isset($edit_vals['0']['journal_slug'])?$edit_vals['0']['journal_slug']:''; ?>" required >
	</div>
</div>
...
<div class="form-group">
	...
	<div class="col-sm-9">
		<select class="form-control" name="archive_volume" required>
			...
			foreach ($volume_arr as $key => $value) {
				if($edit_vals['0']['archive_volume'] == $key) {
...
<div class="form-group">
	...
	<div class="col-sm-9">
		<input type="text" name="archive_fulltext" id="archive_fulltext" class="regular-text" value="<?php echo isset($edit_vals['0']['archive_fulltext'])?$edit_vals['0']['archive_fulltext']:''; ?>" required>
